...
class OpenSubtitles(SubDownloader):
    ...
    def LogIn(self):
        # Log in to server
        loginData = self.osProxy.LogIn(OpenSubtitlesUrl_UserName, OpenSubtitlesUrl_Password, "eng",
    ...
    def SearchSubs(self, fileData, lang):
        ...
        searchResults = self.osProxy.SearchSubtitles(self.token, [
            {'moviehash': epHash, 'moviebytesize': str(fileSizeBytes), 'sublanguageid': lang}])
